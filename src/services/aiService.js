...
// Crop health AI insight (for dashboard cards)
export const getAIInsight = (crop) => {
  if (!crop || !crop.cropType) {
    return {
      status: "monitor",
      message: "Insufficient crop data for AI analysis.",
    };
  }
  ...
  switch (cropType) {
    case "rice":
      return {
        status: "healthy",
        message: "Low pest risk. Suitable for local market.",
      };

    case "wheat":
      return {
        status: "organic",
        message: "Possible nitrogen deficiency detected.",
      };
    ...
    default:
      return {
        status: "monitor",
        message: "Crop requires regular monitoring.",
      };
  }
};
